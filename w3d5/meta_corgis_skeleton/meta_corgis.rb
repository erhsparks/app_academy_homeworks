...
class MetaCorgiPerkPackage
  ...
  #   case name
  #   when :bone, :kibble, :silly_outfit
  #     info = @shopping_list.send("get_#{name}_info", @package_id)
  #     happiness = @shopping_list.send("get_#{name}_happiness", @package_id)
  #
  #     caps_name = name.to_s.split("_").map!(&:capitalize).join(" ")
  #     results = "#{caps_name}: #{info}: #{happiness} licks"
  #   else
  #     super
  #   end
  # end
  ...
  def self.define_perk(name)
    define_method("name") do
      ...
      caps_name = name.split("_").map!(&:capitalize).join(" ")
      results = "#{caps_name}: #{info}: #{happiness} licks"
    end
  end
end
